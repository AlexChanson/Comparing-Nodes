...
FILENAME_REGEX = re.compile(
    r"^(?P<data>[a-zA-Z0-9_-]+)_(?P<heuristic>lp|ls|rd|sls|exp)_(?P<k>\d+)_(?P<run>\d+)\.log$"
)
...
def parse_log_file(path: Path):
    ...
    return dict(
        file=str(path),
        data=data, heuristic=heuristic, k=k, run=run,
        D=D, n=n,
        cpu_time=cpu, wall_time=wall,
        best_score=best_score, clusters=clusters,
...
def aggregate_runs(df: pd.DataFrame):
    ...
    return (
        df.groupby(group_cols, dropna=False, observed=True)
          .agg(cpu_time_mean=("cpu_time","mean"),
...
def compute_score_range_by_data(df: pd.DataFrame):
    ...
    heur = (
        df.groupby(["data","heuristic"], observed=True, as_index=False)
          .agg(mean_best=("best_score","mean"),
...
def compute_mean_ari(df: pd.DataFrame):
    ...
    by_data_heur = (
        per_k.groupby(["data","heuristic"], observed=True, as_index=False)
             .agg(mean_ari=("mean_ari","mean"))
    )
    by_heur = (
        per_k.groupby("heuristic", observed=True, as_index=False)
             .agg(mean_ari=("mean_ari","mean"),
...
def _asym_err_for_scale(mean, std, scale_mode):
    ...
    if scale_mode == "log" and mean > 0:
        ...
        lower = mean - (mean / f)
        upper = (mean * f) - mean
        lower = max(lower, 0.0); upper = max(upper, 0.0)
        return (lower, upper)
    else:
...
def bar_with_std(
    df, *, x, mean_col, std_col,
    hue=None, order=None, hue_order=None,
    title="", xlabel="", ylabel="", out=None,
    yscale="auto"
):
    """
    ...
    """
    sns.set_theme(style=STYLE); sns.set_palette(PALETTE)

    if order is None: order = list(pd.unique(df[x]))
    if hue and (hue_order is None): hue_order = list(pd.unique(df[hue]))

    ax = sns.barplot(
        data=df, x=x, y=mean_col, hue=hue,
        order=order, hue_order=hue_order, errorbar=None
    )
        ...
        for xv in order:
            ...
            for hv in hue_order:
                if hv in present_hues:
                    if patch_i >= len(ax.patches): break
...
def overall_stats(df, group_col, value_col):
    return (
        df.groupby(group_col, observed=True, as_index=False)
          .agg(mean=(value_col,"mean"), std=(value_col,"std"))
    )

def score_range_plot_normalized(ranges_df, heur_df, out, show_error=True):
    sns.set_theme(style=STYLE); sns.set_palette(PALETTE)
...
def score_range_positions_overview(ranges_df, heur_df, out):
    ...
    sns.scatterplot(
        data=h, x="pos", y="data",
        hue="heuristic", hue_order=HEURISTIC_ORDER,
...
def score_boxplot_by_k_heuristic(df_runs: pd.DataFrame, out, yscale="auto"):
    """
    ...
    g = sns.catplot(
        ...
        x="k", y="best_score",
        hue="heuristic", hue_order=HEURISTIC_ORDER,
        showfliers=False, height=5, aspect=1.6, legend=True
    )
    ...
    ax.set_xlabel(XTEXT["k"]); ax.set_ylabel(YTEXT["score"])
    ax.legend(title=XTEXT["heuristic"], bbox_to_anchor=(1.02,1), loc="upper left")
    g.fig.tight_layout(); g.fig.savefig(out, dpi=PLOT_DPI); plt.close(g.fig)

def score_boxplot_per_data(df_runs: pd.DataFrame, outdir: Path, yscale="auto"):
    """
...
def main(indir: Path, outdir: Path):
    ...
    cpu_hd = (agg.groupby(["heuristic","data"], observed=True, as_index=False)
                 .agg(mean=("cpu_time_mean","mean"), std=("cpu_time_mean","std")))
    bar_with_std(cpu_hd, x="heuristic", mean_col="mean", std_col="std",
                 hue="data", order=cpu_order,
                 title=TITLES["cpu_by_heur_data"], xlabel=XTEXT["heuristic"], ylabel=YTEXT["cpu"],
                 out=outdir / "cpu_by_heuristic_and_data.png", yscale="auto")

    bar_with_std(cpu_overall.sort_values("mean"), x="heuristic", mean_col="mean", std_col="std",
                 order=cpu_order,
                 title=TITLES["cpu_by_heur_overall"], xlabel=XTEXT["heuristic"], ylabel=YTEXT["cpu"],
                 out=outdir / "cpu_by_heuristic_overall.png", yscale="auto")

    cpu_kh = (agg.groupby(["k","heuristic"], observed=True, as_index=False)
                 .agg(mean=("cpu_time_mean","mean"), std=("cpu_time_mean","std")))
    bar_with_std(cpu_kh, x="k", mean_col="mean", std_col="std",
                 hue="heuristic", hue_order=HEURISTIC_ORDER,
                 title=TITLES["cpu_by_k_heur"], xlabel=XTEXT["k"], ylabel=YTEXT["cpu"],
                 out=outdir / "cpu_by_k_and_heuristic.png", yscale="auto")

    cpu_Dh = (agg.groupby(["D","heuristic"], observed=True, as_index=False)
                 .agg(mean=("cpu_time_mean","mean"), std=("cpu_time_mean","std")))
    bar_with_std(cpu_Dh, x="D", mean_col="mean", std_col="std",
                 hue="heuristic", hue_order=HEURISTIC_ORDER,
                 title=TITLES["cpu_by_D_heur"], xlabel=XTEXT["D"], ylabel=YTEXT["cpu"],
                 out=outdir / "cpu_by_D_and_heuristic.png", yscale="auto")

    cpu_nh = (agg.groupby(["n","heuristic"], observed=True, as_index=False)
                 .agg(mean=("cpu_time_mean","mean"), std=("cpu_time_mean","std")))
    bar_with_std(cpu_nh, x="n", mean_col="mean", std_col="std",
                 hue="heuristic", hue_order=HEURISTIC_ORDER,
                 title=TITLES["cpu_by_n_heur"], xlabel=XTEXT["n"], ylabel=YTEXT["cpu"],
                 out=outdir / "cpu_by_n_and_heuristic.png", yscale="auto")

    # -------- Wall --------
    wall_overall = overall_stats(agg, "heuristic", "wall_time_mean")
    wall_order = wall_overall.sort_values("mean")["heuristic"].tolist()

    wall_hd = (agg.groupby(["heuristic","data"], observed=True, as_index=False)
                 .agg(mean=("wall_time_mean","mean"), std=("wall_time_mean","std")))
    bar_with_std(wall_hd, x="heuristic", mean_col="mean", std_col="std",
                 hue="data", order=wall_order,
                 title=TITLES["wall_by_heur_data"], xlabel=XTEXT["heuristic"], ylabel=YTEXT["wall"],
                 out=outdir / "wall_by_heuristic_and_data.png", yscale="auto")

    bar_with_std(wall_overall.sort_values("mean"), x="heuristic", mean_col="mean", std_col="std",
                 order=wall_order,
                 title=TITLES["wall_by_heur_overall"], xlabel=XTEXT["heuristic"], ylabel=YTEXT["wall"],
                 out=outdir / "wall_by_heuristic_overall.png", yscale="auto")

    wall_kh = (agg.groupby(["k","heuristic"], observed=True, as_index=False)
                 .agg(mean=("wall_time_mean","mean"), std=("wall_time_mean","std")))
    bar_with_std(wall_kh, x="k", mean_col="mean", std_col="std",
                 hue="heuristic", hue_order=HEURISTIC_ORDER,
                 title=TITLES["wall_by_k_heur"], xlabel=XTEXT["k"], ylabel=YTEXT["wall"],
                 out=outdir / "wall_by_k_and_heuristic.png", yscale="auto")

    wall_Dh = (agg.groupby(["D","heuristic"], observed=True, as_index=False)
                 .agg(mean=("wall_time_mean","mean"), std=("wall_time_mean","std")))
    bar_with_std(wall_Dh, x="D", mean_col="mean", std_col="std",
                 hue="heuristic", hue_order=HEURISTIC_ORDER,
                 title=TITLES["wall_by_D_heur"], xlabel=XTEXT["D"], ylabel=YTEXT["wall"],
                 out=outdir / "wall_by_D_and_heuristic.png", yscale="auto")

    wall_nh = (agg.groupby(["n","heuristic"], observed=True, as_index=False)
                 .agg(mean=("wall_time_mean","mean"), std=("wall_time_mean","std")))
    bar_with_std(wall_nh, x="n", mean_col="mean", std_col="std",
                 hue="heuristic", hue_order=HEURISTIC_ORDER,
                 title=TITLES["wall_by_n_heur"], xlabel=XTEXT["n"], ylabel=YTEXT["wall"],
                 out=outdir / "wall_by_n_and_heuristic.png", yscale="auto")

    # -------- Best score (bar charts) --------
    score_hd = (agg.groupby(["heuristic","data"], observed=True, as_index=False)
                  .agg(mean=("best_score_mean","mean"), std=("best_score_mean","std")))
    bar_with_std(score_hd, x="heuristic", mean_col="mean", std_col="std",
                 hue="data", order=HEURISTIC_ORDER,
                 title=TITLES["score_by_heur_data"], xlabel=XTEXT["heuristic"], ylabel=YTEXT["score"],
                 out=outdir / "score_by_heuristic_and_data.png", yscale="auto")

    score_overall = overall_stats(agg, "heuristic", "best_score_mean")
    bar_with_std(score_overall, x="heuristic", mean_col="mean", std_col="std",
                 order=HEURISTIC_ORDER,
                 title=TITLES["score_by_heur_overall"], xlabel=XTEXT["heuristic"], ylabel=YTEXT["score"],
                 out=outdir / "score_by_heuristic_overall.png", yscale="auto")

    score_kh = (agg.groupby(["k","heuristic"], observed=True, as_index=False)
                  .agg(mean=("best_score_mean","mean"), std=("best_score_mean","std")))
    bar_with_std(score_kh, x="k", mean_col="mean", std_col="std",
                 hue="heuristic", hue_order=HEURISTIC_ORDER,
                 title=TITLES["score_by_k_heur"], xlabel=XTEXT["k"], ylabel=YTEXT["score"],
                 out=outdir / "score_by_k_and_heuristic.png", yscale="auto")

    score_Dh = (agg.groupby(["D","heuristic"], observed=True, as_index=False)
                  .agg(mean=("best_score_mean","mean"), std=("best_score_mean","std")))
    bar_with_std(score_Dh, x="D", mean_col="mean", std_col="std",
                 hue="heuristic", hue_order=HEURISTIC_ORDER,
                 title=TITLES["score_by_D_heur"], xlabel=XTEXT["D"], ylabel=YTEXT["score"],
                 out=outdir / "score_by_D_and_heuristic.png", yscale="auto")

    score_nh = (agg.groupby(["n","heuristic"], observed=True, as_index=False)
                  .agg(mean=("best_score_mean","mean"), std=("best_score_mean","std")))
    bar_with_std(score_nh, x="n", mean_col="mean", std_col="std",
                 hue="heuristic", hue_order=HEURISTIC_ORDER,
                 title=TITLES["score_by_n_heur"], xlabel=XTEXT["n"], ylabel=YTEXT["score"],
                 out=outdir / "score_by_n_and_heuristic.png", yscale="auto")

    # -------- Box plots: all data & per data --------
    score_boxplot_by_k_heuristic(
        df_runs=df,
        out=outdir / "score_box_by_k_and_heuristic.png",
        yscale="auto"
    )
    score_boxplot_per_data(
        df_runs=df,
        outdir=outdir,
        yscale="auto"
    )
    ...
    score_range_plot_normalized(ranges_df, heur_df,
                                out=outdir / "score_range_by_data_normalized.png",
                                show_error=True)
    score_range_plot_normalized(ranges_df, heur_df,
                                out=outdir / "score_range_by_data_normalized_noerr.png",
                                show_error=False)
    score_range_positions_overview(
        ranges_df, heur_df,
        out=outdir / "scoreRange_norm_overview.png"
    )
    ...
    g1 = sns.catplot(
        data=df1, kind="bar",
        x="data", y="mean_ari", hue="heuristic",
        hue_order=HEURISTIC_ORDER, errorbar=None,
        height=5, aspect=1.6, legend=True
    )
    if getattr(g1, "_legend", None) is not None:
        g1._legend.remove()
    ax1 = g1.axes[0,0]
    ax1.set_title(TITLES["ari_by_data_heur"])
    ax1.set_xlabel(XTEXT["data"]); ax1.set_ylabel(YTEXT["ari"])
    ax1.legend(title=XTEXT["heuristic"], bbox_to_anchor=(1.02,1), loc="upper left")
    g1.fig.tight_layout(); g1.fig.savefig(outdir / "ari_by_data_and_heuristic.png", dpi=PLOT_DPI); plt.close(g1.fig)
    ...
    bar_with_std(tmp, x="heuristic", mean_col="mean", std_col="std",
                 order=HEURISTIC_ORDER,
                 title=TITLES["ari_by_heur"], xlabel=XTEXT["heuristic"], ylabel=YTEXT["ari"],
                 out=outdir / "ari_by_heuristic_overall.png", yscale="linear")
