...
def plot_pushdown_by_label(csv_path: str) -> None:
    ...
    if missing:
        msg = f"Missing required columns after normalization: {missing}"
    ...
    for m in ("time_total", "time_indicators", "time_validation"):
        if m not in time_cols:
            msg = f"Required time column '{m}' not found. Available: {time_cols}"
            raise ValueError(msg)

    # 5) Aggregate: mean & std over runs for each (label, pushdown, ratio)
    keys = ["label", "pushdown", "ratio_prop_dropped"]
    mean_df = df.groupby(keys, as_index=False)[time_cols].mean().rename(columns={c: f"{c}_mean" for c in time_cols})
    std_df = df.groupby(keys, as_index=False)[time_cols].std(ddof=1).rename(columns={c: f"{c}_std" for c in time_cols})
    agg = mean_df.merge(std_df, on=keys, how="inner").sort_values(keys).reset_index(drop=True)
    ...
    for metric in ("time_total", "time_indicators", "time_validation"):
        ...
        if m_mean not in agg.columns or m_std not in agg.columns:
            msg = f"Expected '{m_mean}' and '{m_std}' in aggregated data."
            raise ValueError(msg)
    ...
    def _lineplot_with_errorbars(plot_df, metric, filename, title):
        ...
        for (lbl, pdwn), sub in subset.groupby(["label", "pushdown"]):
            ...
            plt.errorbar(sub["ratio"], sub["mean"], yerr=sub["std"], fmt="none", capsize=4, linewidth=0.8, alpha=0.9)
